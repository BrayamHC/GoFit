...
export class ClientesRepoHelper {
    static aplicarFiltros(query: Knex.QueryBuilder, filtros: any): Knex.QueryBuilder {
        ...
        if (filtros.status) query.where('c.status', filtros.status);

        if (filtros.nombre) {
            filtros.exacto
                ? query.whereRaw('LOWER(c.nombre) = LOWER(?)', [filtros.nombre])
                : query.whereILike('c.nombre', `%${filtros.nombre}%`);
        }

        if (filtros.apellido) {
            filtros.exacto
                ? query.whereRaw('LOWER(c.apellido) = LOWER(?)', [filtros.apellido])
                : query.whereILike('c.apellido', `%${filtros.apellido}%`);
        }

        if (filtros.email) query.whereILike('c.email', `%${filtros.email}%`);
        if (filtros.telefono) query.where('c.telefono', filtros.telefono);
