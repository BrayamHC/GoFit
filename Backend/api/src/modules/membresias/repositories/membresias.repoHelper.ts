...
export class MembresiasRepoHelper {
    static aplicarFiltros(query: Knex.QueryBuilder, filtros: any): Knex.QueryBuilder {
        ...
        if (filtros.membresia_id) query.where('m.membresia_id', filtros.membresia_id);
        if (filtros.status) query.where('m.status', filtros.status);
        if (filtros.tipo) query.where('m.tipo', filtros.tipo);
        if (filtros.moneda) query.where('m.moneda', filtros.moneda);
        if (filtros.dias_duracion) query.where('m.dias_duracion', Number(filtros.dias_duracion));
        if (filtros.precio_min) query.where('m.precio', '>=', Number(filtros.precio_min));
        if (filtros.precio_max) query.where('m.precio', '<=', Number(filtros.precio_max));
        if (filtros.nombre) query.whereILike('m.nombre', `%${filtros.nombre}%`);
