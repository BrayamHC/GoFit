  ...
  <PageContainer>
    ...
    <v-row align="center" class="mb-5 mb-md-7">
      <v-col>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">Gestión de membresías del gimnasio</p>
      </v-col>
      <v-col cols="auto">
        <v-btn
          color="red-darken-3"
    ...
    <v-row v-else>
        ...
        <v-card rounded="xl" elevation="0" border height="480" class="d-flex flex-column">
          <v-card-text class="pa-0 flex-grow-1 d-flex flex-column">
            ...
            <v-row no-gutters class="pa-5 pb-4">
              <v-col>
                <v-row no-gutters align="center" class="mb-3">
                  <v-chip
                    :color="membresia.status === 'activo' ? 'green' : 'red'"
                    size="x-small"
                    label
                    variant="tonal"
            ...
            <v-row dense no-gutters class="pa-4 pb-0">
              <v-col
                v-for="recurso in getRecursos(membresia)"
                :key="recurso.label"
                cols="6"
                class="pa-1"
              >
            ...
            <div class="pa-4 pb-2 flex-grow-1">
              <template v-for="n in 4" :key="n">
                <v-row
                  v-if="membresia.caracteristicas && membresia.caracteristicas[n - 1]"
                  no-gutters
                  align="center"
                  class="mb-2"
                >
      ...
      <!-- Empty state -->
      <v-col v-if="membresiasFiltradas.length === 0" cols="12">
              ...
              <v-row dense>
                ...
                <v-col cols="6">
                  <v-select
                    v-model="form.tipo"
                    label="Tipo"
                    :items="['mensual', 'trimestral', 'semestral', 'anual', 'personalizado']"
                    variant="outlined"
                    density="compact"
                    hide-details
                  />
              ...
              <v-card rounded="lg" elevation="2" max-width="280" class="mx-auto">
                ...
                <v-card-text class="pa-5">
                  <v-row no-gutters align="center" class="mb-3">
                    ...
                    >
                      <v-icon color="red-darken-3" size="22">mdi-dumbbell</v-icon>
                    </v-sheet>
                    <div>
                      ...
                      <p class="text-caption text-medium-emphasis mb-0">
                        {{ form.descripcion || 'Descripción corta' }}
                      </p>
                    </div>
                  </v-row>
...
const membresiasFiltradas = computed(() => {
  if (filtroActivo.value === 'todos') return store.membresias
  return store.membresias.filter((m) => m.status === filtroActivo.value)
})

const contarPorFiltro = (key) => {
  if (key === 'todos') return store.membresias.length
  return store.membresias.filter((m) => m.status === key).length
}

const formatPrecio = (precio) => parseFloat(precio ?? 0).toFixed(2)

const getRecursos = (m) => [
  { icon: 'mdi-calendar-range', label: 'Duración', valor: `${m.dias_duracion} días` },
  { icon: 'mdi-clock-check-outline', label: 'Días de gracia', valor: `${m.dias_gracia ?? 0} días` },
  { icon: 'mdi-currency-usd', label: 'Moneda', valor: m.moneda },
  { icon: 'mdi-identifier', label: 'ID', valor: `#${m.membresia_id}` },
]
...
const guardarMembresia = async () => {
  const result = await store.crearMembresia({ ...form.value })
...
watch(
  ...
  (tipo) => {
    ...
    if (dias !== null) form.value.dias_duracion = dias
  },
)

onMounted(() => {
  if (store.membresias.length === 0) store.fetchMembresias()
})
</script>
